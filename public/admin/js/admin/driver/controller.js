"use strict";
angular.module("app.admin.driver", [
    'app.driver.services',
    'app.core.services.LocalStorage'
])
        .controller("DriverController", ['$scope', '$rootScope', 'DriverService', 'LocalStorageService', 'NgTableParams', '$uibModal', '$stateParams', '$state', '$http',
            function($scope, $rootScope, DriverService, LocalStorageService, NgTableParams, $uibModal, $stateParams, $state, $http) {
    ...
    $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                ...
                $scope.getDriverList = function(page, searchQuery) {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {
                            return DriverService.getDriverList(offset, searchQuery, oprid).then(function(data) {
                ...
                $scope.addDriver = function(driver) {
                    if (driver._id == undefined) {
                        ...
                        DriverService.addNewDriver(driver).then(function(data) {
                            ...
                            $state.reload('admin.driver');
                            $state.go('admin.driver', {id: oprid}, {reload: true, location: 'replace', inherit: false, notify: true});
                        ...
                        DriverService.updateDriver(driver).then(function(data) {
                            ...
                            $scope.$broadcast('start', true);
                            $state.reload('admin.driver');
                            $state.go('admin.driver', {id: data.operator}, {reload: true, inherit: false, notify: true});
