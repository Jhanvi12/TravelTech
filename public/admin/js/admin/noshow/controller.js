"use strict";
angular.module("app.admin.noshow", [
    'app.noshow.services',
    'app.core.services.LocalStorage'
])
        .controller("NoshowController", ['$scope', '$location', '$rootScope', 'NoshowRideService', 'LocalStorageService', 'NgTableParams', '$stateParams', '$state', '$http',
            function($scope, $location, $rootScope, NoshowRideService, LocalStorageService, NgTableParams, $stateParams, $state, $http) {
        ...
        $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                 ...
                 **/
                $scope.getnoshowRidesList = function(page, type, searchQuery) {
                    var offset;
                    if (!page) {
                        offset = 0;
                    } else {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {
                            return NoshowRideService.getnoshowRidesList(offset, type, searchQuery).then(function(data) {
                 ...
                 **/
                $scope.NoshowRide = function() {
                    console.log("In no show");
                    if ($stateParams.id) {
                        NoshowRideService.NoshowRide($stateParams.id).then(function(data) {
                            if (data) {
                                $scope.lead = data;
                            }
                        }).catch(function(err) {
                ...
                $scope.performAction = function() {
                    ...
                    if (actionToPerform == "delete") {
                        NoshowRideService.deleteCanceledRide(inputJsonString)
                                .then(function(data) {
                                    $scope.$broadcast('start', true);
                                    $scope.getnoshowRidesList();
                                    $scope.$broadcast('stop', true);
                                }).catch(function(err) {
                ...
                $scope.applyGlobalSearch = function() {
                    $scope.getnoshowRidesList(1, "", $scope.globalSearchTerm);
                };

                /*Passing the value for sorting of data */
                $scope.goValue = function(value) {
                    $scope.value = value;
                    $scope.getnoshowRidesList(1, $scope.value, "");
                };
                ...
                $scope.getBidsList = function() {
                    if ($stateParams.id) {
                       NoshowRideService.fetchBidsForRide($stateParams.id).then(function(data) {
                            if (data) {
                ...
                $scope.noshowridesexportcsv = function() {
                        NoshowRideService.noshowridesexportcsv().then(function(data) {
                            console.log(data);
                            if (data) {
                                $scope.user = data;
                                if(window.open() == null){
                                    ...
                                    window.open($location.protocol()+'://'+$location.host()+':'+$location.port()+'/public/uploads/noshowridesfile.csv');
                                }
                                // $scope.filename="noshowridesfile.csv";
                            }                            
                        }).catch(function(err) {
                            console.log('Error');
                        });
