angular.module('app.noshow.services', [])
        .service('NoshowRideService', ['$http', '$q', function($http, $q) {

                this.getnoshowRidesList = function(offset, type, query) {
                    var deffered = $q.defer();
                    var path = "";
                    if (type) {
                        var path = '/api/v1/rides?status=noshow&all=true&offset=' + offset + '&type=' + type;
                    } else if (query) {
                        var path = '/api/v1/rides?status=noshow&all=true&offset=' + offset + '&city=' + query;
                    } else {
                        var path = '/api/v1/rides?status=noshow&all=true&offset=' + offset;
                    }
