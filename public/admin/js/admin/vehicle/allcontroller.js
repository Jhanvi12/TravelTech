"use strict";
angular.module("app.admin.listvehicle", [
    'app.vehicle.services',
    'app.core.services.LocalStorage'
])
        .controller("AllVehicleController", ['$scope','$location', 'ngToast', '$rootScope', 'VehicleService', 'LocalStorageService', 'NgTableParams', '$uibModal', '$stateParams', '$state', '$http',
            function($scope, $location, ngToast,  $rootScope, VehicleService, LocalStorageService, NgTableParams, $uibModal, $stateParams, $state, $http) {
           ...
           $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                ...
                $scope.getVehicleList = function(page, searchQuery) {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {
                            return VehicleService.getAllVehicleList(offset, searchQuery, $stateParams.id).then(function(data) {
                                if (data) {
                ...
                $scope.addVehicle = function(vehicle) {
                    if (vehicle._id == undefined) {
                        ...
                        $scope.vehicle.category = $scope.vehicle.category._id;
                        VehicleService.addNewVehicle(vehicle).then(function(data) {
                            $scope.$broadcast('start', true);
                            $state.reload('admin.listvehicle');
                            $state.go('admin.listvehicle', {}, {reload: true, location: 'replace', inherit: false, notify: true});
                        ...
                        VehicleService.updateVehicleInfo(vehicle).then(function(data) {
                            $scope.$broadcast('start', true);
                            $state.reload('admin.listvehicle');
                            $state.go('admin.listvehicle', {}, {reload: true, inherit: false, notify: true});
                ...
                $scope.vehiclesexportcsv = function() {
                        VehicleService.vehiclesexportcsv().then(function(data) {
                            console.log(data);
                            if (data) {
                                $scope.user = data;
                                if(window.open() == null){
                                    ...
                                    window.open($location.protocol()+'://'+$location.host()+':'+$location.port()+'/public/uploads/vehiclesfile.csv');
                                }
                                // $scope.filename="vehiclesfile.csv";
                            }
                        }).catch(function(err) {
                            console.log('Error');
                        });
