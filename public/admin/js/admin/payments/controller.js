"use strict";
angular.module("app.admin.payment", [
    'app.payment.services',
    'app.core.services.LocalStorage',
    'app.core.service.Count'
])
        .controller("PaymentsController", ['$scope', '$rootScope', 'PaymentService', 'LocalStorageService', 'NgTableParams', '$stateParams', 'Count', '$state', '$http',
            function($scope, $rootScope, PaymentService, LocalStorageService, NgTableParams, $stateParams, Count, $state, $http) {
        ...
        $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                 ...
                 **/
                $scope.getPaymentsList = function(page, status, searchQuery) {
                    var offset;
                    if (!page) {
                        offset = 0;
                    } else {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {                            
                            return PaymentService.getPaymentsList(offset, status, searchQuery).then(function(data) {
                ...
                $scope.getNotification = function() {
                    if ($stateParams.id) {
                        NotificationService.getNotification($stateParams.id).then(function(data) {
                            if (data) {
                                $scope.review = data;
                            }
                        }).catch(function(err) {
                ...
                $scope.applyGlobalSearch = function() {
                    $scope.getNotificationsList(1, $scope.globalSearchTerm);
                };
