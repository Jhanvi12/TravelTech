...
module.exports = function(req, res) {
    ...
    defaultVehicleService.getDefaultVehiclesByCategories(req.params.id).then(function(data) {
        if (!data) {
            return response(res, null, "no content", 204);
        }
        return  response(res, data, "default vehicles of category fetched", 200);
    }).catch(function(err) {
