...
module.exports = function(req, res) {
    ...
    rideService.getOpenLeadCsvFile().then(function(user) {
        if (!user) {
        ...
        json2csv({
            ...
            fs.writeFile('public/uploads/opeanleadsfile.csv', csv, function(err) {
                if (err) throw err;
                console.log('file saved');
                res.download('public/uploads/opeanleadsfile.csv');
            });
        })
    }).catch(function(err) {
