"use strict";
angular.module("app.admin.upcomingride", [
    'app.upcomingride.services',
    'app.core.services.LocalStorage'
])
        .controller("UpcomingRidesController", ['$scope', 'ngToast', '$location', '$rootScope', 'UpcomingRideService', 'LocalStorageService', 'NgTableParams', '$stateParams', '$state', '$http',
            function($scope, ngToast, $location, $rootScope, UpcomingRideService, LocalStorageService, NgTableParams, $stateParams, $state, $http) {
        ...
        $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                 ...
                 **/
                $scope.getUpcomingRidesList = function(page, type, searchQuery) {
                    var offset;
                    if (!page) {
                        offset = 0;
                    } else {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {
                            return UpcomingRideService.getUpcomingRidesList(offset, type, searchQuery).then(function(data) {
                                $scope.currentPage = page ? page : 1;
                                $scope.data = data.data;
                                $scope.recordFetchedLength = data.count;
                                console.log("The record length",$scope.recordFetchedLength);
                 ...
                 **/
                $scope.getUpcomingRide = function() {
                    if ($stateParams.id) {
                        UpcomingRideService.getUpcomingRide($stateParams.id).then(function(data) {
                            if (data) {
                                $scope.lead = data;
                            }
                        }).catch(function(err) {
                ...
                $scope.performAction = function() {
                    ...
                    if (actionToPerform == "delete") {
                        UpcomingRideService.deleteUpcomingRide(inputJsonString)
                                .then(function(data) {
                                    $scope.$broadcast('start', true);
                                    $scope.getUpcomingRidesList();
                                    $scope.$broadcast('stop', true);
                                }).catch(function(err) {
                ...
                $scope.applyGlobalSearch = function() {
                    $scope.getUpcomingRidesList(1, "", $scope.globalSearchTerm);
                };

                /*Passing the value for sorting of data */
                $scope.goValue = function(value) {
                    $scope.value = value;
                    $scope.getUpcomingRidesList(1, $scope.value, "");
                };
                ...
                $scope.getAprrovedBid = function() {
                    UpcomingRideService.approvedBidsForRide($stateParams.id).then(function(data) {
                        var bidArray = [];
                        var totalAmount = 0;
                        $scope.bidArray = data;
                        console.log("The bid array is", $scope.bidArray);
                        angular.forEach(data, function(val) {
                         ...
                         console.log(item.vehicle.category);
                            });
                        });
                ...
                $scope.upcomingridesexportcsv = function() {
                        UpcomingRideService.upcomingridesexportcsv().then(function(data) {
                            console.log(data);
                            if (data) {
                                $scope.user = data;
                                if(window.open() == null){
                                    ...
                                    window.open($location.protocol()+'://'+$location.host()+':'+$location.port()+'/public/uploads/upcomingridesfile.csv');
                                }
                                // $scope.filename="upcomingridesfile.csv";
                            }
                        }).catch(function(err) {
                            console.log('Error');
                        });
