"use strict";
angular.module("app.admin.completedride", [
    'app.completedride.services',
    'app.core.services.LocalStorage'
])
        .controller("CompletedRidesController", ['$scope','$location', 'ngToast', '$rootScope', 'CompletedRideService', 'LocalStorageService', 'NgTableParams', '$stateParams', '$state', '$http',
            function($scope, $location, ngToast, $rootScope, CompletedRideService, LocalStorageService, NgTableParams, $stateParams, $state, $http) {
            ...
            $http.defaults.headers.common['Authorization'] = 'Bearer ' + $scope.admin.token;
                 ...
                 **/
                $scope.getCompletedRidesList = function(page, type, searchQuery) {
                    var offset;
                    if (!page) {
                        offset = 0;
                    } else {
                    ...
                    $scope.tableParams = new NgTableParams({}, {counts: [], getData: function() {
                            return CompletedRideService.getCompletedRidesList(offset, type, searchQuery).then(function(data) {
                 ...
                 **/
                $scope.getCompletedRide = function() {
                    if ($stateParams.id) {
                        CompletedRideService.getCompletedRide($stateParams.id).then(function(data) {
                            if (data) {
                                $scope.lead = data;
                            }
                        }).catch(function(err) {
                ...
                $scope.performAction = function() {
                    ...
                    if (actionToPerform == "delete") {
                        CompletedRideService.deleteCompletedRide(inputJsonString)
                                .then(function(data) {
                                    $scope.$broadcast('start', true);
                                    $scope.getCompletedRidesList();
                                    $scope.$broadcast('stop', true);
                                }).catch(function(err) {
                ...
                $scope.applyGlobalSearch = function() {
                    $scope.getCompletedRidesList(1, "", $scope.globalSearchTerm);
                };

                /*Passing the value for sorting of data */
                $scope.goValue = function(value) {
                    $scope.value = value;
                    $scope.getCompletedRidesList(1,$scope.value,"");
                };
                ...
                $scope.getAprrovedBid = function() {
                    CompletedRideService.approvedBidsForRide($stateParams.id).then(function(data) {
                        var bidArray = [];
                        var totalAmount = 0;
                        $scope.bidArray = data;
                ...
                $scope.completedridesexportcsv = function() {
                        CompletedRideService.completedridesexportcsv().then(function(data) {
                            console.log(data);
                            if (data) {
                                $scope.user = data;
                                if(window.open() == null){
                                    ...
                                    window.open($location.protocol()+'://'+$location.host()+':'+$location.port()+'/public/uploads/completedridesfile.csv');
                                }
                                // $scope.filename="completedridesfile.csv";
                            }
                        }).catch(function(err) {
                            console.log('Error');
                        });
